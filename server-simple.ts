...
app.post('/lessons', async (req, res) => {
  try {
    const { title, description, islandId, emoji, courseId, bookId } = req.body;
    ...
    const result = await pool.query(`
      INSERT INTO lessons (title, description, island_id, emoji, status, order_index, course_id, book_id)
      VALUES ($1, $2, $3, $4, 'draft', $5, $6, $7)
      RETURNING *
    `, [title, description || null, islandId || null, emoji || '🏝️', nextOrder, courseId || null, bookId || null]);
...
app.get('/books/:id/lessons', async (req, res) => {
  try {
    const { id } = req.params;
    const result = await pool.query(
      'SELECT * FROM lessons WHERE book_id = $1 ORDER BY order_index, created_at',
      [id]
    );
    res.json({ success: true, data: result.rows });
  } catch (error) {
