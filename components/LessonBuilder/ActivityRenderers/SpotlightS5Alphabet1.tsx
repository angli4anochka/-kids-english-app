...
export default function SpotlightS5Alphabet1({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'alphabet-1-done' || submitted || isTeacher || !lessonId) return;
    try {
