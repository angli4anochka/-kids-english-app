...
export default function SpotlightEx18({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  const [submitted, setSubmitted] = useState(false);

  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'ex18-done' || submitted || isTeacher || !lessonId) return;
    try {
  ...
  return (
    <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative' }}>
      <iframe src="/spotlight/ex18.html" style={{ width: '100%', height: '100%', border: 'none', display: 'block' }} scrolling="no" />
      {submitted && (
