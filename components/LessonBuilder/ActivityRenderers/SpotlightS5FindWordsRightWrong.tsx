...
export default function SpotlightS5FindWordsRightWrong({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'find-words-right-wrong-done' || submitted || isTeacher || !lessonId) return;
    try {
