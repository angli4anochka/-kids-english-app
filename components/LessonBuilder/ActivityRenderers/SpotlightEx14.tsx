...
export default function SpotlightEx14({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'ex14-done' || submitted || isTeacher || !lessonId) return;
    try {
  ...
  return (
    <div ref={containerRef} style={{ width: '100%', height: '100%', overflow: 'hidden', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#fff', position: 'relative' }}>
      <div style={{ width: W, height: H, transform: `scale(${scale})`, transformOrigin: 'center center', flexShrink: 0 }}>
        <iframe src="/spotlight/ex14.html" width={W} height={H} style={{ border: 'none', display: 'block' }} scrolling="no" />
      </div>
      {submitted && (
