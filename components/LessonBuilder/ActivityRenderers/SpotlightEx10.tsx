...
export default function SpotlightEx10({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  // Listen for "all done" message from iframe
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'ex10-done' || submitted || isTeacher || !lessonId) return;
    try {
