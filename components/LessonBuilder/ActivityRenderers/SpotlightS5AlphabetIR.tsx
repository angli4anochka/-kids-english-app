...
export default function SpotlightS5AlphabetIR({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'alphabet-ir-done' || submitted || isTeacher || !lessonId) return;
    try {
