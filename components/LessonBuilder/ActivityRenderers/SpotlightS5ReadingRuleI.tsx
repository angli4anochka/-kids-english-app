...
export default function SpotlightS5ReadingRuleI({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'reading-rule-i-names-done' || submitted || isTeacher || !lessonId) return;
    try {
