...
export default function SpotlightEx20({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  const [submitted, setSubmitted] = useState(false);

  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'ex20-done' || submitted || isTeacher || !lessonId) return;
    try {
  ...
  return (
    <div style={{ width: '100%', height: '100%', overflow: 'hidden', position: 'relative' }}>
      <iframe src="/spotlight/ex20.html" style={{ width: '100%', height: '100%', border: 'none', display: 'block' }} scrolling="no" />
      {submitted && (
