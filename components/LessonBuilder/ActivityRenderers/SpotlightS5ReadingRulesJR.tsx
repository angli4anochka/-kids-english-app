...
export default function SpotlightS5ReadingRulesJR({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'reading-rules-jr-done' || submitted || isTeacher || !lessonId) return;
    try {
