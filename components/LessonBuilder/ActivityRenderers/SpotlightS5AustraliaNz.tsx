...
export default function SpotlightS5AustraliaNz({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'australia-nz-done' || submitted || isTeacher || !lessonId) return;
    try {
