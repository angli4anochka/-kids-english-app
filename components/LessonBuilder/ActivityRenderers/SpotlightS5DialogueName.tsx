...
export default function SpotlightS5DialogueName({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'dialogue-whats-your-name-done' || submitted || isTeacher || !lessonId) return;
    try {
