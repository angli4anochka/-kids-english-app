...
export default function SpotlightEx40({ isTeacher, lessonId, activityId, sessionId }: Props) {
  ...
  const src = `/spotlight/ex40.html${params.toString() ? '?' + params.toString() : ''}`;
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
              <thead>
                <tr style={{ background: '#f5f5f5', position: 'sticky', top: 0 }}>
                  ...
                  <th style={{ padding: '6px 10px', textAlign: 'left', borderBottom: '1px solid #e0e0e0' }}>Ответы</th>
                </tr>
              </thead>
              <tbody>
                {results.map(r => {
                  ...
                  return (
                    <>
                      <tr key={r.id} style={{ borderBottom: '1px solid #f0f0f0', cursor: 'pointer' }}
                        ...
                        <td style={{ padding: '5px 10px' }}>
                          <div style={{ display: 'flex', gap: 3 }}>
                            {Array.from({ length: TOTAL }, (_, i) => {
                              ...
                              const color = !a?.value ? '#ddd' : a.correct ? '#4caf50' : '#e53935';
                              return <div key={i} title={`${i + 1}: ${a?.value || '—'}`}
                                style={{ width: 14, height: 14, borderRadius: 3, background: color }} />;
                            })}
                            <span style={{ fontSize: 11, color: '#888', marginLeft: 4 }}>{isExp ? '▲' : '▼'}</span>
                          </div>
                        </td>
                      </tr>
                      {isExp && (
                          ...
                          <td colSpan={3} style={{ padding: '6px 12px' }}>
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 2, fontSize: 12 }}>
                              {Array.from({ length: TOTAL }, (_, i) => {
                                const a = answers['q' + (i + 1)];
                                return (
                                  <span key={i} style={{ color: !a?.value ? '#bbb' : a.correct ? '#2e7d32' : '#c62828' }}>
                                    {i + 1}. {a?.value || '—'}
                                  </span>
                                );
                              })}
