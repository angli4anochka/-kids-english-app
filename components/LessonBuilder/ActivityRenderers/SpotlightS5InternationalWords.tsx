...
export default function SpotlightS5InternationalWords({ isTeacher, lessonId, activityId, sessionId }: Props) {
  const { user } = useAuth();
  ...
  const handleMessage = useCallback(async (e: MessageEvent) => {
    if (e.data?.type !== 'international-words-done' || submitted || isTeacher || !lessonId) return;
    try {
