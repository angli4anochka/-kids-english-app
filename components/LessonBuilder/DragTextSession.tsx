...
interface DragTextSessionProps {
  activityText: string;
  lessonId: string;
...
export default function DragTextSession({
  activityText,
  lessonId,
  activityIndex,
  groupId,
  onClose
}: DragTextSessionProps) {
  const { user } = useAuth();
  ...
  const words = parseText(activityText);
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
        <div className="bg-white rounded-2xl p-8">
          ...
          <p className="text-gray-700 text-center">Создание сессии...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="fixed inset-0 bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex items-center justify-center z-50 p-8">
        <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full p-8">
          ...
          >
            {session.participants && session.participants.length > 0
              ? `Начать активность (${session.participants.length} уч.)`
              : 'Ожидание учеников...'}
          </button>
        </div>
      </div>
  ...
  if (session.status === 'completed') {
    const avgScore = session.participants?.length
      ? Math.round(
          session.participants.reduce((sum, p) => sum + (p.score || 0), 0) /
        ...
        <div className="bg-white rounded-3xl shadow-2xl max-w-6xl w-full p-8">
          ...
          <div className="text-center mb-8">
            ...
            <p className="text-2xl text-gray-600">
              Средний результат: <span className="font-bold text-green-600">{avgScore}%</span>
            </p>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b-2 border-gray-300">
                  ...
                  <th className="text-center py-3 px-4 font-bold text-gray-700">Результат</th>
